-- =============================================================================
-- DYNAMIC WEEKLY STREAK SYSTEM + EARNING CORRECTION
-- =============================================================================
-- This migration:
-- 1. Replaces the old claim_daily_login_reward with a new version that reads
--    weeklyTotalNgn / weeklyTotalUsd from system_settings and distributes via
--    arithmetic progression across 7 days.
-- 2. Runs a one-time correction script that recalculates all existing streak
--    earnings against the new formula, deducting any overpayment.
-- =============================================================================
...
DECLARE
  ...
  v_a numeric;  -- first term of arithmetic progression
  v_d numeric;  -- common difference
  v_day_index integer;
BEGIN
  ...
  IF v_is_global THEN
    ...
    -- Global users: balance is stored in NGN-equivalent, so multiply by dollar_price
  ...
  -- Formula: S = 7a + 21d = v_total_weekly
  -- We choose a = Total * 4 / 49 and d = Total / 49
  -- This gives a nice progressive curve: Day1 = 4T/49, Day7 = 10T/49
...
-- =============================================================================
-- EARNING CORRECTION SCRIPT
-- Recalculates every user's streak earnings using the new arithmetic progression
-- and deducts any overpayment from their wallet.
-- Only affects login_reward type transactions — NOT referral, article, or other earnings.
-- =============================================================================
DO $$
DECLARE
  v_rec record;
...
BEGIN
  ...
  LOOP
    -- Get this user's actual total from transaction log (authoritative source)
    ...
    -- If no actual transactions, skip
    IF v_actual_total <= 0 THEN
      CONTINUE;
    END IF;

    -- Determine the plan-specific weekly total
    IF v_settings IS NOT NULL AND v_settings ? v_rec.plan_id THEN
    ...
    -- Calculate what they SHOULD have earned for v_i days
    v_a := v_total_weekly * 4.0 / 49.0;
    v_d := v_total_weekly / 49.0;

    v_num_complete_weeks := v_i / 7;
    v_remaining_days := v_i % 7;

    -- Sum of one full week = 7*a + 21*d = v_total_weekly (by design)
    ...
    -- Only deduct if they were overpaid by more than 1 unit
    IF v_diff > 1 THEN
      -- Deduct from wallet
      UPDATE public.wallet_balances
