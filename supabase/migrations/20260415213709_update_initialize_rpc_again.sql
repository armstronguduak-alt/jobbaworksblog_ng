...
create or replace function public.initialize_my_account(
  ...
  _country_code text default '+234',
...
begin
  ...
  on conflict (user_id) do update
    set email = coalesce(excluded.email, profiles.email),
        ...
        is_global = coalesce(excluded.is_global, profiles.is_global),
        updated_at = now();
...
revoke all on function public.initialize_my_account(text, text, text, text, text, text, text, text, text, boolean) from public;
grant execute on function public.initialize_my_account(text, text, text, text, text, text, text, text, text, boolean) to authenticated;
