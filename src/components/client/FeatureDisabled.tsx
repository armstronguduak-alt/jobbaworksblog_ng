...
export function FeatureDisabled() {
  ...
  const path = pathname.split('/')[1] || 'This feature';
  ...
  return (
    <div className="flex-grow flex flex-col items-center justify-center min-h-[60vh] px-4 text-center bg-surface w-full">
      ...
      <div className="flex gap-4">
        <Link href="/" className="px-6 py-3 bg-primary text-white font-bold rounded-xl shadow-md hover:bg-emerald-800 transition-colors">
          Go Home
        </Link>
        <Link href="/dashboard" className="px-6 py-3 bg-surface-container text-on-surface font-bold rounded-xl border border-surface-container-high hover:bg-surface-variant transition-colors">
          Dashboard
        </Link>
      </div>
    </div>
