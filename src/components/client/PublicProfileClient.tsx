  ...
  const { data, isLoading, isPending } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: pData } = await supabase
        .from('profiles')
        .select('user_id, username, name, avatar_url, bio, followers_count, following_count, gender, is_verified')
          ...
          <div className="flex flex-col md:flex-row items-center gap-6">
            ...
            <div className="text-center md:text-left">
              <h1 className="text-3xl font-black font-headline text-slate-900 flex items-center justify-center md:justify-start gap-2">
                {profile.name}
                {(profile as any).is_verified && <span className="material-symbols-outlined text-blue-500" style={{ fontVariationSettings: "'FILL' 1" }}>verified</span>}
              </h1>
