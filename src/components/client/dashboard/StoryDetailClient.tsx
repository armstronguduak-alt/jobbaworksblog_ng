    ...
    <div className="bg-white min-h-screen font-body pb-20">
        ...
        <div className="p-6">
           ...
           <div className="space-y-4">
             {chapters.length === 0 ? (
               ...
               chapters.map((ch, i) => (
                 <Link key={ch.id} href={`/stories/read/${story.slug}/${ch.chapter_number}`} className="flex items-center gap-4 py-3 border-b border-slate-100 group hover:pl-2 transition-all">
      ...
      {chapters.length > 0 && (
        <div className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-md border-t border-slate-200 p-4 z-50 flex justify-center shadow-[0px_-5px_20px_rgba(0,0,0,0.05)]">
           <Link href={`/stories/read/${story.slug}/1`} className="w-full max-w-lg bg-[#6c5ce7] text-white py-4 rounded-full font-bold text-center text-lg hover:bg-[#5b4bcc] transition-colors shadow-lg">
             Start Reading
           </Link>
        </div>
      )}
