...
export function StoriesHubClient() {
        ...
        <section>
           ...
           <div className="space-y-4">
             {trendingStories.map((story, i) => (
               <Link key={story.id} href={`/stories/${story.slug}`} className="flex gap-4 items-center group">
         ...
         {filteredAllStories.length === 0 ? (
           ...
           <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 md:gap-6">
              {filteredAllStories.map(story => (
                <Link key={story.id} href={`/stories/${story.slug}`} className="group hover:-translate-y-1 transition-transform">
