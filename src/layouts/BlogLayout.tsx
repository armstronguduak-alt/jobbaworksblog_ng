...
export function BlogLayout() {
  ...
  return (
    <div className="min-h-screen bg-surface text-on-surface antialiased flex flex-col font-body">
      ...
      <aside className={`
        fixed inset-y-0 left-0 z-[70] w-[270px] bg-white shadow-2xl flex flex-col
      ...
      `}>
        <div className="flex items-center justify-between p-4 border-b border-surface-container">
          ...
          <button onClick={() => setIsMenuOpen(false)} className="p-1.5 text-emerald-950 bg-emerald-50 rounded-xl">
            <X size={18} />
          </button>
        </div>
        <nav className="flex-1 overflow-y-auto px-3 py-4 flex flex-col gap-1">
          <Link to="/" onClick={() => setIsMenuOpen(false)} className="font-bold text-emerald-700 px-4 py-3 bg-emerald-50 rounded-xl flex items-center gap-3">
            <span className="material-symbols-outlined text-[18px]">home</span>Home
          </Link>
          <Link to="/plans" onClick={() => setIsMenuOpen(false)} className="font-semibold text-emerald-950 px-4 py-3 hover:bg-surface-container rounded-xl flex items-center gap-3">
            <span className="material-symbols-outlined text-[18px]">credit_card</span>Plans
          </Link>
          <Link to="/category/business" onClick={() => setIsMenuOpen(false)} className="font-semibold text-emerald-950 px-4 py-3 hover:bg-surface-container rounded-xl flex items-center gap-3">
            <span className="material-symbols-outlined text-[18px]">business_center</span>Business
          </Link>
          <Link to="/category/technology" onClick={() => setIsMenuOpen(false)} className="font-semibold text-emerald-950 px-4 py-3 hover:bg-surface-container rounded-xl flex items-center gap-3">
            <span className="material-symbols-outlined text-[18px]">computer</span>Technology
          </Link>
          <Link to="/category/entertainment" onClick={() => setIsMenuOpen(false)} className="font-semibold text-emerald-950 px-4 py-3 hover:bg-surface-container rounded-xl flex items-center gap-3">
            <span className="material-symbols-outlined text-[18px]">movie</span>Entertainment
          </Link>
          <Link to="/category/health" onClick={() => setIsMenuOpen(false)} className="font-semibold text-emerald-950 px-4 py-3 hover:bg-surface-container rounded-xl flex items-center gap-3">
            <span className="material-symbols-outlined text-[18px]">favorite</span>Health
          </Link>
          <Link to="/category/fitness" onClick={() => setIsMenuOpen(false)} className="font-semibold text-emerald-950 px-4 py-3 hover:bg-surface-container rounded-xl flex items-center gap-3">
        ...
        <div className="p-4 border-t border-surface-container">
          {user ? (
            <button onClick={() => { signOut(); setIsMenuOpen(false); }} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-rose-50 text-rose-600 font-bold transition-colors">
              <span className="material-symbols-outlined text-[18px]">logout</span>
              Logout
            </button>
          ) : (
